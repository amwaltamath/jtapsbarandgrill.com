...
import { generateGoogleWalletUrl, isGoogleWalletConfigured, computeTier } from '../../../lib/wallet';
...
export const GET: APIRoute = async ({ request }) => {
  ...
  if (!isGoogleWalletConfigured()) {
    return new Response(
      JSON.stringify({ error: 'Google Wallet is not configured on this server.' }),
  ...
  try {
    const saveUrl = await generateGoogleWalletUrl(member);

    // Record the wallet pass in the database
    const serial = `jtaps-google-${user.id}`;
    await supabaseAdmin
      .from('wallet_passes')
      .upsert(
        {
    ...
    console.error('Google Wallet URL generation error:', err);
    return new Response(
