...
// Helper: validate Bearer token and check admin status
...
// Helper: generate campaign key from subject + message
function getCampaignKey(subject: string, message: string): string {
  return crypto.createHash('sha256').update($|src/pages/api/{message}).digest('hex');
}
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const { action = 'send', subject, message, subscribers, useProgress = false, batchSize = 50 } = body;
    ...
    // send action - send individual emails
    let sent = 0;
    let failed = 0;
    ...
    for (const subscriber of subscribers) {
      try {
        ...
        sent++;
        // Respect rate limit (2 requests per second = 600ms between requests)
        await new Promise(resolve => setTimeout(resolve, 600));
      } catch (err) {
        console.error(Failed to send email to src/pages/api/{subscriber.email}:, err);
        failed++;
      }
    }
